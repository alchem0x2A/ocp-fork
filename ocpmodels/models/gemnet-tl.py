...
class GemNetT(torch.nn.Module):
        ...
        if self.freeze:
            for name, param in self.named_parameters():
                if param.requires_grad:
                    # Disable on any free layer
                    if all([regex not in name for regex in unfreeze_layers]):
                    # if "out_blocks.3" not in name:
                    # and \
                       # "out_blocks.1" not in name and \
                       # "int_blocks.2" not in name and \
                       # "int_blocks.1" not in name and \
                       # "int_blocks.0" not in name:
    ...
    def forward(self, data):
        ...
        #if self.freeze:
        #E_t, F_st = self.after_freeze_OB[0](h, m, rbf_out, idx_t)
